...
const ProductEdit = () => {
    ...
    const [avatar_new, setAvatarNew] = useState('');
    ...
    const handleFormUpdate = async (e) => {
        ...
        // formData.append('avatar_new', avatar_new);
        ...
        //
        try {
            const response = await axios.post('http://127.0.0.1:8000/api/products-update/' + id, formData, {
                headers: {
    ...
    useEffect(() => {
        fetch('http://127.0.0.1:8000/api/products-edit/' + id, {
            ...
            .then((data) => {
                ...
                setBrand_id(data.products.brand_id);
                setDesigner_id(data.products.designer_id);
                setCategory_id(data.products.category_id);
                //
    ...
    const handleChangeInput = (event) => {
        switch (event.target.name) {
            ...
            case "color":
                ...
                break;
            case "avatar_new":
                setAvatarNew(event.target.files[0]);
                break;
            case "status":
        ...
        <Box m="20px">
            ...
            <form onSubmit={handleFormUpdate}>
                ...
                >
                    ...
                    <div className="mb-3">
                        <label for="avatar_new" className="form-label">Ảnh đại diện mới: (Nếu cần)</label>
                        <input class="form-control" type="file" id="avatar_new" name="avatar_new"
                               onChange={handleChangeInput}/>
                        <p style={{color: "red"}}>
                            {errorsField && errorsField.avatar_new}
                        </p>
                    </div>
